...
async function apply(ctx) {
  ...
  async function handler(invocation) {
    ...
    switch (sub) {
      ...
      default:
        return { kind: 'error', text: `未知子命令：${sub}（可用 dir / play [关键词] / pause / next / prev / vol / refresh）` };
    }
  }
    ...
    if (commands !== undefined && commands !== null) {
      disposers.push(
        commands.register({
          ...
          input: { hint: '[dir <路径>|play [关键词]|pause|next|prev|vol <0-100>|refresh]' },
          handler,
        }),
      );
    }
