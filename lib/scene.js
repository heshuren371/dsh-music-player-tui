 ...
 * @param {{ setDir(dir: string): Promise<void>, refresh(): Promise<void> }} controller
 *   场景触发的宿主动作：换目录并扫描 / 重新扫描当前目录。
 */
  ...
  return function MusicScene({ React, ui, close }) {
      ...
      listChildren = visible.map((track, offset) => {
        ...
        const props = isSelected
          ...
          : isCurrent
            ...
            : { color: 'text' };
        return e(Text, { key: track.id, ...props }, line);
      });
    }
    ...
    if (mode === 'search') {
      ...
      bottomLine = e(
        ...
        truncate('enter 播放 · space 暂停 · j/k 选择 · n/p 切歌 · s 排序 · l 循环 · / 搜索 · u 目录 · q/Esc 退出', width),
      );
    }
